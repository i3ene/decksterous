...
MarketplaceRoutes.get('/all', [middleware.findAll({ model: Marketplace, scopes: ["inventoryItem"] })], controller.result(Marketplace));

MarketplaceRoutes.get("/", [middleware.find({ model: Marketplace, scopes: ["inventoryItem"] })], controller.result(Marketplace));
