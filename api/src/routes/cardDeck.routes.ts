...
CardDeckRoutes.post("/item", [auth.isAdmin, middleware.get({
  model: CardDeck,
  body: {key: ["deck", "id"]}
}), middleware.get({model: InventoryItem, body: {key: ["item", "id"]}}), middleware.addAssociation({
  model: CardDeck,
  association: {name: "inventoryItems", data: InventoryItem}
})], controller.message("last"));

CardDeckRoutes.post("/items", [auth.isAdmin, middleware.get({
  model: CardDeck,
  body: {key: ["deck", "id"]}
}), middleware.getAll({model: InventoryItem, list: {key: 'items', id: 'id'}}), middleware.addAssociation({
  model: CardDeck,
  association: {name: "inventoryItems", data: InventoryItem}
})], controller.message("last"));

CardDeckRoutes.delete("/item", [auth.isAdmin, middleware.get({
  model: CardDeck,
  body: {key: ["deck", "id"]}
}), middleware.get({model: InventoryItem, body: {key: ["item", "id"]}}), middleware.removeAssociation({
  model: CardDeck,
  association: {name: "inventoryItems", data: InventoryItem}
})], controller.message("last"));

CardDeckRoutes.delete("/items", [auth.isAdmin, middleware.get({
  model: CardDeck,
  body: {key: ["deck", "id"]}
}), middleware.getAll({model: InventoryItem, list: {key: 'items', id: 'id'}}), middleware.removeAssociation({
  model: CardDeck,
  association: {name: "inventoryItems", data: InventoryItem}
})], controller.message("last"));
